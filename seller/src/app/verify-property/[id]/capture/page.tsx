...
//   const VERIFICATION_STEPS = [...baseSteps, ...customSteps];
...
//     localStorage.setItem(`custom_steps_${propertyId}`, JSON.stringify(updatedCustomSteps));
...
//       console.log("📡 Dispatching combined assets with coordinates to backend proxy layer...");
//       const serverResponse = await axios.post("/api/admin-bulk-approve", {
//         propertyId: propertyId,
//         photos: formattedPhotosArray,
//         fileKeys: allVerifiedFileKeys,
...
export default function PropertyCameraCapturePage() {
  ...
  useEffect(() => {
    async function fetchPropertyDetails() {
      try {
        ...
        console.error("Failed to fetch property details schema:", error);
      } finally {
        setLoading(false);
      }
    }
  ...
  const handleAddMoreImages = () => {
    ...
    const newCustomStep = {
      id: `custom_photo_${Date.now()}`,
      label: `Extra Photo ${nextCustomIdx}`,
      target: 'additional dynamic user perspective tracking capture layer mapping asset frame alignment checking configuration.'
    };
  ...
  const verifyWithAI = async (imageSrc: string, stepId: string) => {
    ...
    try {
      ...
      if (result?.backendDebugLogs) {
        console.log("%c📜 [SERVER-SIDE CORE PIPELINE EXECUTION TRACE]", "background: #333; color: #ffeb3b; font-size: 12px; padding: 3px;");
        console.table(result.backendDebugLogs);
      }
  ...
  const handleFinalSubmit = async () => {
    try {
      ...
      try {
        const tokenExtractResponse = await axios.get("/api/get-token");
        sellerToken = tokenExtractResponse?.data?.accessToken || tokenExtractResponse?.data?.token || "";
      } catch (tokenErr) {
        console.warn("Extraction utility routing bypass:", tokenErr);
      }

      if (!sellerToken && typeof window !== "undefined") {
        sellerToken = localStorage.getItem("token") || localStorage.getItem("accessToken") || "";
      }
      ...
      if (!serverResponse.data.success) {
        throw new Error(serverResponse.data.message || "Unified proxy engine rejected submission.");
      }
  ...
  return (
    <div className="min-h-screen bg-white flex flex-col p-4 max-w-md mx-auto relative pb-24">
      ...
      <DuplicateBlockModal 
        isOpen={showDuplicateModal} 
        onClose={() => setShowDuplicateModal(false)}
        message="Error: This property configuration is already listed inside our database grid registry map!"
      />
    </div>
  );
}
...
export default function LeadSummaryListClient() {
  ...
  const handleCrmDashboard = async (lead: LeadItem, ownerRole?: string) => {
    const firstProperty = lead.propertyContacts?.[0]?.property;
    const propertyContact = lead.propertyContacts?.[0];
    const propertyId = propertyContact?.propertyId || firstProperty?.id;
    ...
    // Background fetch initialization fallbacks
    // let dynamicOwnerName = "Prerit Ahuja";
    // let dynamicOwnerPhone = "9354040527";
    let dynamicOwnerName;
    let dynamicOwnerPhone;
    ...
    try {
      if (propertyId) {
        try {
          const propResponse = await axiosInstance.get<any>(
            `https://kmaglobalproperty.com/api/backend/end-user/properties/${propertyId}`
          );
          
          const targetUser = propResponse?.data?.property?.user;
          if (targetUser) {
            dynamicOwnerName = targetUser.name;
            dynamicOwnerPhone = targetUser.phone;
          }
        } catch (fetchErr) {
          console.warn("⚠️ End-user deep property lookup failed, using fallbacks.", fetchErr);
        }
      }

      const propertyAny = (firstProperty ?? {}) as Record<string, unknown>;
      const formattedTitle = stripHtml(firstProperty?.title || "Unknown Property");
      ...
      const payload = {
        customer: {
          ...
          email: lead.email?.trim() || "no-email@example.com",
          phone: (() => {
            const rawCustomerPhone = (lead.phone || "").replace(/\D/g, ""); 
            if (!rawCustomerPhone || rawCustomerPhone === "0000000000") return "0000000000";
            
            if (rawCustomerPhone.startsWith("91") && rawCustomerPhone.length > 10) {
              return `+${rawCustomerPhone}`;
            }
            
            return `+91${rawCustomerPhone}`;
          })(),
          website_user_id: lead.id || `lead-${Date.now()}`,
        },
        
        channel_partner: {
          name: dynamicOwnerName,
          mobile_no: (() => {
            const rawPhone = dynamicOwnerPhone.replace(/\D/g, ""); 
            if (!rawPhone || rawPhone === "null") return "NA";
          ...
          role: getString(ownerRole, getString(propertyAny.ownerRole, "CHANNEL_PARTNER")),
        },

        property: {
          available_for: getString(propertyAny.availableFor, getString(lead.buildingType)),
          property_type: getString(lead.propertyTypes?.[0]),
          property_name: formattedTitle,
          property_sub_type: getString(propertyAny.propertySubType),
          zone: getString(propertyAny.zone),
          sector: getString(lead.locations?.[0]),
          bhk: getString(propertyAny.bhk),
          ...
          buildup_area: getNumber(propertyAny.buildUpArea),
          availability_by: getString(propertyAny.availabilityBy),
          maintenance_cost: getNumber(propertyAny.maintenanceChargeAmount ?? propertyAny.maintenanceCost),
          security: getNumber(propertyAny.securityDepositAmount ?? propertyAny.security),
          amount: getNumber(firstProperty?.price ?? firstProperty?.monthlyRent),
          brokerage: getNumber(propertyAny.brokerage),
          no_of_bedroom: getNumber(propertyAny.numberOfBedroom),
          no_of_washroom: getNumber(propertyAny.numberOfWashroom),
          ...
          total_floor: getNumber(propertyAny.totalFloor),
          floor_no: getNumber(propertyAny.floorNo),
          furnish_status: getString(propertyAny.furnishStatus ?? propertyAny.furnishType),
          facing: getString(propertyAny.facing),
          quality_rating: getString(propertyAny.qualityRating),
          basic_amenities: Array.isArray(propertyAny.basicAmenities)
            ? propertyAny.basicAmenities.join(", ")
          ...
          website_property_id: propertyId || "UNKNOWN_PROPERTY",
        },
      };

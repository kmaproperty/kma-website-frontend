...
//   const isPending = isOwnerCpPending;
...
export default function LoginCard() {
  ...
  const { mutate: sendOwnerCpOtp, isPending: isOwnerCpPending } = useMutation({
    ...
    onSuccess: (response) => {
      ...
      console.log("[Frontend LoginCard] NestJS API Success Response Object:", response);

      if (response.otp) {
        console.log(`[Frontend LoginCard] Dispatching OTP code [${response.otp}] to local email API route...`);
        
        axios.post("/api/login-email-otp", {
          phone: mobileInput.value, 
          otpCode: response.otp,     
        })
        .then((localRes) => {
          console.log("[Frontend LoginCard] Local Email API Success:", localRes.data);
        })
        .catch((err) => {
          console.error("[Frontend LoginCard] Local Email API Route 400 Error Context:", {
        ...
        console.warn("[Frontend LoginCard] 'otp' key is missing or undefined in NestJS api response payload structure!");
      }
  ...
  const handleContinue = () => {
    ...
    // console.log("[Testing] Triggering manual agreement PDF dispatch from LoginCard...");
    
    // axios.post("/api/send-agreement-pdf", {
    //   userId: "0a681a09-714d-4faa-9ece-5e093962de9a",
    //   email: "prerit.dev@gmail.com"           
    // })
    // .then((res) => {
    //   console.log("[Testing Success] API Route Response:", res.data);
    //   toast.success("Test PDF Email dispatched successfully!");
    // })
    // .catch((err) => {
    //   console.error("[Testing Failure] API Route Error:", err.response?.data || err.message);
    //   toast.error("Test PDF Email failed: " + (err.response?.data?.message || err.message));
    // });

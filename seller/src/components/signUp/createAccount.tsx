...
//         }
//       }
//       // Channel Partner code is auto-generated by the backend on register,
//       // so no input validation is needed here.
//     }
...
export default function CreateAccount({ step }: { step: number }) {
  ...
  const validateStep = useCallback(() => {
    ...
    if (step === 1) {
      ...
      // Owner Step 1: City and Zone are required
      if (userData?.role === USER_TYPE.OWNER) {
        if (!formData.city) {
          errors.city = "City is required";
        }
        if (!(formData as any).zone) {
    ...
    if (step === 2) {
      // Channel Partner Step 2: City and Zone are required
      if (
        (Array.isArray(formData.city) && formData.city.length === 0) ||
        !formData.city
      ) {
    ...
    if (userData?.role === USER_TYPE.OWNER) {
      if (validateStep()) {
        ...
        let payload: any = {
          ...
          intent: (propertyIntent ?? LIST_TYPE.SELL) as ListType,
          phone: userData.phone,
          city: (formData.city as any)?.label ?? "",
          zone: (formData as any).zone?.value ?? "",
        };
          ...
          <div className="flex flex-col gap-3">
            {step === 1 && (
              <>
                ...
                {/* OWNER STEP 1: City & Zone */}
                {userData?.role === USER_TYPE.OWNER && (
                  <>
                    <p className="required-label text-sm 1xl:text-base text-text-black">
                      City
                    </p>
                    ...
                    <p className="required-label text-sm 1xl:text-base text-text-black mt-2">
                      Select Zone
                    </p>
                    <div>
                      <DynamicSelect
                        isMulti={false}
                        isError={Boolean(formErrors.zone)}
                        placeholder={"Select Zone"}
                        onChange={(value: OptionType) => {
                          dispatch(setFormField({ key: "zone", value }));
                          setFormErrors((prev) => ({ ...prev, zone: "" }));
                        }}
                        options={ZONE_OPTIONS}
                        value={(formData as any).zone}
                        minHeight={"47px"}
                        fontwidth={"14px"}
                      />
                      {formErrors.zone && (
          ...
          <div className="w-full sm:w-[460px] bg-white rounded-[14px] p-6 sm:p-8 text-center">
            ...
            <div className="mt-6 flex flex-col sm:flex-row gap-3">
              <button
                ...
                onClick={() => {
                  if (generatedCpCode && typeof navigator !== "undefined" && navigator.clipboard) {
                    navigator.clipboard.writeText(generatedCpCode).catch(() => {});
                    toast.success("Code copied to clipboard");
                  }
                }}
                className="w-full sm:flex-1 h-11 rounded-full border border-[#010048] text-sm font-medium text-[#010048] hover:bg-[#0100481A]"

...
export default function UserKyc({ tabName, event }) {
  ...
  const { data: kycDetails, refetch: refreshKyc, isLoading: detailsLoader } = useQuery({
    ...
    select: (response: KycStatusResponse) => {
      return response;
    },
    staleTime: 0,
    refetchOnMount: true,
  ...
  // 🎯 1. COMPRESSED 2-STEP LIST FOR CHANNEL PARTNER & OWNER
  // Aadhaar and Bank Details fields are safely truncated from client-side visualization loop
  const kycList = isOwner
    ? [{ icon: AgreementIcon, name: "Agreement Signature" }]
    : [
        { icon: PhotoIcon, name: "Photo Upload" },
  ...
  const handleActiveTab = (tabName: string, index: number) => {
    // Dynamically tracking steps scaling context
    let allowNavigation = false;
    if (kycDetails) {
      if (kycDetails.kyc_steps_completed >= index) {
        allowNavigation = true;
      }
      if (index === 1 && kycDetails.kyc_steps_completed >= 1) {
        allowNavigation = true;
      }
    }

    if (!allowNavigation && index > 0) {
      return;
    }
  ...
  const renderTitle = () => {
    ...
    return titleList[activeStep as keyof typeof titleList] || "KYC Verification";
  };
  ...
  useEffect(() => {
    ...
    refreshKyc();
  }, [tabName, defaultTab, refreshKyc]);

  return (
    <div className="bg-white rounded-xl p-5 lg:p-10 w-full">
      {!isKycComplete && (
        <div className="flex items-center justify-center gap-8 md:gap-16 h-[100px] max-w-xl mx-auto">
          {kycList.map((item, index) => {
            const Icon = item.icon;
            const isActive = activeStep === item.name;

            return (
              <div
                key={item.name}
                onClick={() => handleActiveTab(item.name, index)}
                className={`flex justify-between items-center flex-col transition-colors cursor-pointer ${
                  isActive ? 'text-blue' : 'text-[#888888]'
                }`}
              >
                <div className="flex justify-between items-center flex-col gap-2 p-2">
                ...
                {isActive && <div className="border-b-2 w-full border-blue"></div>}
              </div>
            );
          })}
        </div>
      ...
      <div className="w-full flex flex-col py-4 items-center gap-6">
        ...
        {isKycComplete && event === "signing_complete" && (
          <p className="text-green-600 text-xl font-semibold">
            Document signed successfully! Redirecting to dashboard...
          </p>
        )}
      </div>
    </div>

...
// import { getPriceTrendData } from "@/utils/priceTrendHelper";
...
//   const isRentalProperty = useMemo(() => {
//     const listingType = asString(propertyDetails?.listingTypeName)?.toLowerCase() || "";
//     const monthlyRent = asNumber(propertyDetails?.monthlyRent);
//     return listingType.includes("rent") || Boolean(monthlyRent && monthlyRent > 0);
//   }, [propertyDetails]);

//   // Price Trend Metric Calculations
//   const pricePerSqFtVal = useMemo(() => {
//     return (
//       asNumber(propertyDetails?.pricePerSqFt) ??
//       asNumber(propertyDetails?.ratePerSqFt) ??
//       (asNumber(propertyDetails?.price) && asNumber(propertyDetails?.buildUpAreaSqFt)
//         ? Math.round(Number(propertyDetails?.price) / Number(propertyDetails?.buildUpAreaSqFt))
//         : 13550)
//     );
//   }, [propertyDetails]);

//   const localityNameForTrend = useMemo(() => {
//     return (
//       asString(propertyDetails?.localityName) ??
//       asString(detailsResponse?.location?.locality) ??
//       asString(propertyDetails?.address) ??
//       "Sector 106 Gurgaon"
//     );
...
//     return getPriceTrendData(localityNameForTrend, pricePerSqFtVal);
//   }, [localityNameForTrend, pricePerSqFtVal]);
...
//         if (tab.id === "furnishing") return hasFurnishingSection;
//         if (tab.id === "amenities") return hasAmenitiesSection;
//         if (tab.id === "priceTrends") return !isRentalProperty;
//         return true;
//       }),
//     [hasAmenitiesSection, hasFurnishingSection, isRentalProperty],
//   );
//   useEffect(() => {
//     if (!availableSectionTabs.some((tab) => tab.id === activeSectionTab)) {
//       setActiveSectionTab(availableSectionTabs[0]?.id ?? "overview");
...
//                     </section>
//                   {!isRentalProperty && (
//                     <section className="scroll-mt-32 md:px-0 px-4" id="priceTrends" ref={setSectionRef("priceTrends")}>
//                       <PriceTrendSection
//                         societyName={propertyTitle}
//                         currentPrice={pricePerSqFtVal}
...
//         if (!name) return null;
//         const rawId =
//           typeof rec.id === "string" && rec.id ? rec.id : `${name}-${idx}`;
//         return { id: rawId, name };
//       }
//       return null;
//     })
...
//   const isRentalProperty = useMemo(() => {
//     const listingType =
//       asString(propertyDetails?.listingTypeName)?.toLowerCase() || "";
//     const monthlyRent = asNumber(propertyDetails?.monthlyRent);
//     return (
//       listingType.includes("rent") || Boolean(monthlyRent && monthlyRent > 0)
//     );
//   }, [propertyDetails]);
...
//       asNumber(propertyDetails?.pricePerSqFt) ??
//       asNumber(propertyDetails?.ratePerSqFt) ??
//       (asNumber(propertyDetails?.price) &&
//       asNumber(propertyDetails?.buildUpAreaSqFt)
//         ? Math.round(
//             Number(propertyDetails?.price) /
//               Number(propertyDetails?.buildUpAreaSqFt),
//           )
//         : 13550)
//     );
//   }, [propertyDetails]);
...
//       locality?.cityId ??
//       society?.cityId ??
//       (propertyDetails as { cityId?: string })?.cityId ??
//       "";

//     const pType =
//       propertyDetails?.propertyType &&
//       typeof propertyDetails.propertyType === "object"
//         ? (propertyDetails.propertyType as { id?: string })
//         : undefined;
//     const propertyTypeId =
//       pType?.id ??
//       (propertyDetails as { propertyTypeId?: string })?.propertyTypeId ??
//       undefined;
...
//   const setFavorite = useProjectsStore((s) => s.setFavorite);
//   const {
//     mutate: updateSimilarFavorite,
//     isPending: isSimilarFavoriteUpdating,
//   } = useMutation({
//     mutationFn: async ({
//       propertyId,
//       nextIsFavorite,
...
//         queryKey: ["end-user-properties-similar"],
//       });
//       queryClient.invalidateQueries({
//         queryKey: [FAVORITE_PROPERTIES_QUERY_KEY],
//       });
//     },
//     onError: (_err, variables) => {
//       setFavorite(variables.propertyId, !variables.nextIsFavorite);
...
//   const monthlyRent = asNumber(propertyDetails?.monthlyRent);
//   const salePrice =
//     asNumber(propertyDetails?.price) ?? asNumber(propertyDetails?.plotPrice);
//   const currentPriceLabel =
//     monthlyRent && monthlyRent > 0
//       ? `Rs ${formatInr(monthlyRent)}`
//       : salePrice && salePrice > 0
...
//   const quickFactsData = useMemo(() => {
//     const bedrooms =
//       asString(propertyDetails?.bhkTypeName) ??
//       asString(propertyDetails?.bhkType);
//     const furnishing = asString(propertyDetails?.furnishType);
//     const bathrooms =
...
//     const area =
//       asNumber(propertyDetails?.buildUpAreaSqFt) ??
//       asNumber(propertyDetails?.builtUpArea);
//     const facing = asString(propertyDetails?.facing);
//     const floor = asNumber(propertyDetails?.floorNumber);
//     const totalFloors =
//       asNumber(propertyDetails?.totalFloorCount) ??
//       asNumber(propertyDetails?.totalFloors);
...
//     if (bathrooms)
//       facts.push({
//         icon: <Bath strokeWidth={1.5} className="h-5 w-5" />,
...
//           />
//         ),
//         label: `${formatInr(area)} Sq. Ft (Built-up Area)`,
//       });
//     if (facing)
//       facts.push({
//         icon: <Trees strokeWidth={1.5} className="h-5 w-5" />,
//         label: facing,
//       });
//     if (floor && totalFloors)
//       facts.push({
//         icon: <Building2 className="h-5 w-5" strokeWidth={1.5} />,
//         label: `${floor}th Floor out of ${totalFloors} Floors`,
//       });
//     return facts;
//   }, [propertyDetails]);
...
//       [
//         "Project Name",
//         asString(propertyDetails?.projectName) ??
//           asString(detailsResponse?.location?.society),
//       ],
//       [
//         "Area",
//         asNumber(propertyDetails?.buildUpAreaSqFt)
...
//     return fromApi.filter(([, value]) => value != null) as Array<
//       [string, string]
//     >;
//   }, [propertyDetails]);
...
//         rating: r.overallRating,
//         review:
//           [r.likeText, r.dislikeText].filter(Boolean).join(" ") || "No comment",
//         avatar: "/assets/profile.png",
//       }));
//     }
//     return [];
...
//       const step = card.offsetWidth + gap;
//       const nextIndex =
//         Math.round(container.scrollLeft / Math.max(step, 1)) + 1;
//       setActiveMobileImageIndex(
//         Math.min(Math.max(nextIndex, 1), resolvedGalleryImages.length),
//       );
//     };
...
//     Record<
//       string,
//       Array<{ name: string; distance: string; address: string | null }>
//     >
//   >({});
//   const [nearbyLoading, setNearbyLoading] = useState(false);

//   const [activeSectionTab, setActiveSectionTab] =
//     useState<ListingSectionId>("overview");
//   const tabsNavRef = useRef<HTMLElement | null>(null);
//   const sectionRefs = useRef<Record<ListingSectionId, HTMLElement | null>>({
//     overview: null,
//     furnishing: null,
...
//   });
//   const setSectionRef =
//     (sectionId: ListingSectionId) => (element: HTMLElement | null) => {
//       sectionRefs.current[sectionId] = element;
//     };
//   const furnishingsCounts = Array.isArray(propertyDetails?.furnishingsCounts)
//     ? (propertyDetails.furnishingsCounts as Array<{
//         item: string;
//         count: number;
//       }>)
//     : [];
...
//           .filter((entry) => entry.isIntersecting)
//           .sort((a, b) => b.intersectionRatio - a.intersectionRatio);
//         const topVisibleSection = visibleEntries[0]?.target.id as
//           | ListingSectionId
//           | undefined;
//         if (topVisibleSection) setActiveSectionTab(topVisibleSection);
//       },
...
//   const activeLocalityPlaces = nearbyPlacesCache[activeLocalityCategory] ?? [];
//   const specialistName = asString(apiChannelPartner?.name) ?? "KMA Property";
//   const specialistFirm =
//     asString(apiChannelPartner?.firmName) ?? "KMA Expert Pro";
//   const specialistImage =
//     toFullAssetUrl(apiChannelPartner?.profileImage) || "/assets/profile.png";
...
//       postedBy: apiChannelPartner ? "channel_partner" : "owner",
//       listingIntent:
//         (propertyDetails as any)?.listingType === "Rent" ? "rent" : "sale",
//       priceValue: 0,
//       priceLabel: "",
//       images: [],
//       agent: {
...
//   const { mutate: submitQuickContact, isPending: isQuickSubmitting } =
//     useMutation({
//       mutationFn: submitEndUserPropertyContactAction,
//       onSuccess: (response) => {
//         toast.success(response?.message ?? "Request submitted successfully");
...
//               </h2>
//               <p className="text-text-gray text-sm mb-6">
//                 You have viewed 3 free properties. Please login or sign up to
//                 continue exploring more properties.
//               </p>
//               <button
//                 onClick={() =>
//                   router.push(
//                     `/user-flow?isLogin=true&redirect=/projects/${params?.projectId}/${listingId}`,
//                   )
//                 }
//                 className="w-full bg-blue text-white py-3 px-6 rounded-full font-medium hover:bg-blue/90 transition cursor-pointer"
//               >
...
//               </button>
//               <nav
//                 className="min-w-0 text-xs font-medium text-text-gray"
//                 aria-label="Breadcrumb"
//               >
//                 <p className="truncate">
//                   Home /{" "}
//                   {propertyTitle
//                     ? propertyTitle.replace(/<[^>]*>/g, "").trim()
//                     : ""}
//                 </p>
//                 <p className="truncate pt-0.5 font-semibold text-[#0A0E67] underline underline-offset-2">
//                   {propertyTitle
...
//             </div>
//             <nav
//               className="hidden text-xs text-white/90 sm:text-sm md:block"
//               aria-label="Breadcrumb"
//             >
...
//                 className="mt-5 mb-3 flex md:hidden items-center justify-center gap-2 rounded-full border border-[#010048] bg-white px-2 py-1 text-sm font-semibold text-[#010048] hover:bg-gray-50 transition cursor-pointer"
//               >
...
//                   <div className="shrink-0 text-right">
//                     <p className="text-[20px] font-bold leading-none text-blue sm:text-3xl">
//                       {currentPriceLabel}
//                     </p>

//                     {/* 🌟 Conditional Rendering: Rental vs Buy (EMI Calculator) */}
//                     {isRentalProperty ? (
//                       <>
...
//                         className="mt-2.5 inline-flex items-center gap-1.5 rounded-full border border-blue/30 bg-blue/5 px-3 py-1 text-xs font-semibold text-blue hover:bg-blue/10 transition cursor-pointer shadow-2xs"
//                       >
...
//               <section className="mt-4 flex min-w-0 flex-col gap-3 md:flex-row md:gap-3 md:px-0 px-4">
//                 <div className="mb-1 flex items-center justify-between md:hidden">
//                   <h3 className="text-[18px] font-semibold text-text-black">
//                     Images
//                   </h3>
//                 </div>
//                 <div
...
//                     className="inline-flex max-w-full min-w-0 shrink-0 items-center gap-2 rounded-md border border-border bg-background-gray px-2.5 py-2 pr-4 text-[13px] text-text-black md:shrink"
//                   >
//                     <span className="flex h-[30px] w-[30px] shrink-0 items-center justify-center rounded-md bg-white">
//                       {fact.icon}
//                     </span>
//                     <span className="min-w-0 break-words">{fact.label}</span>
//                   </div>
...
//                         type="button"
//                         onClick={() => scrollToSection(tab.id)}
//                         className={`whitespace-nowrap border-b-2 px-3 py-3 text-xs transition sm:px-5 sm:py-4 sm:text-sm ${
...
//                                   {label}
//                                 </p>
//                                 <p className="mt-0.5 text-sm font-medium text-text-black">
//                                   {value}
//                                 </p>
//                               </div>
//                             ))}
...
//                     {Array.isArray(propertyDetails?.furnishingsCounts) &&
//                     propertyDetails.furnishingsCounts.length > 0 ? (
//                       <section
//                         id="furnishing"
//                         ref={setSectionRef("furnishing")}
//                         className="scroll-mt-32 md:mx-0 mx-4"
...
//                         <div
//                           id="furnishing-content"
//                           className={
//                             isFurnishingOpen
//                               ? "mt-4 md:mt-4"
//                               : "hidden md:mt-4 md:block"
//                           }
//                         >
//                           <div className="overflow-hidden rounded-lg border border-[#D4D5D8] bg-white md:hidden">
//                             <div className="grid grid-cols-2">
//                               {propertyDetails.furnishingsCounts.map(
//                                 (
//                                   f: { item: string; count: number },
//                                   idx: number,
//                                 ) => (
//                                   <div
//                                     key={`furnishing-mobile-${f.item}-${idx}`}
...
//                         )}
//                       </button>
//                       <h2 className="hidden text-lg font-semibold text-text-black sm:text-xl md:block">
//                         Locality
//                       </h2>
//                       <div
//                         id="locality-content"
//                         className={isLocalityOpen ? "block" : "hidden md:block"}
//                       >
//                         <div className="relative mt-3 h-[200px] overflow-hidden rounded-tl-lg rounded-tr-lg border border-[#D4D5D8] bg-[#ECEEF3] sm:h-[260px] md:h-[300px]">
//                           {lat && lng ? (
//                             <>
...
//                               const Icon = category.icon;
//                               const isActive =
//                                 activeLocalityCategory === category.key;
...
//                                   type="button"
//                                   onClick={() =>
//                                     setActiveLocalityCategory(category.key)
//                                   }
//                                   className={`inline-flex shrink-0 items-center gap-2 rounded-md border px-4 py-2.5 text-sm font-medium transition ${
//                                     isActive
//                                       ? "border-[#05085E] bg-[#05085E] text-white"
//                                       : "border-[#D4D5D8] bg-[#fff] text-text-black hover:bg-white"
//                                   }`}
...
//                           <div className="grid grid-cols-1 md:grid-cols-2">
//                             {nearbyLoading ? (
//                               <p className="py-5 text-sm text-text-gray">
...
//                                           {place.name}
//                                         </p>
//                                         <span className="md:block hidden mt-1 block text-[#888888]">
//                                           {place.distance}
//                                         </span>
//                                       </div>
//                                     </div>
//                                     <span className="md:hidden block text-[#888888]">
//                                       {place.distance}
//                                     </span>
//                                   </a>
//                                 );
...
//                     {Array.isArray(propertyDetails?.amenitiesList) &&
//                     propertyDetails.amenitiesList.length > 0 ? (
//                       <section
//                         id="amenities"
//                         ref={setSectionRef("amenities")}
//                         className="scroll-mt-32 rounded-none md:rounded-xl w-[90%] md:w-[100%] mx-auto"
...
//                         >
//                           <div className="grid grid-cols-2 gap-3 md:hidden">
//                             {propertyDetails.amenitiesList.map(
//                               (name: string) => (
//                                 <div
//                                   key={`amenity-mobile-${name}`}
//                                   className="inline-flex items-center gap-2 rounded-lg border border-[#E2E5EC] bg-white px-3 py-2.5"
...
//                               <Image
//                                 src={
//                                   apiChannelPartner?.profileImage ||
//                                   apiOwnerDetails?.profileImage ||
//                                   "/assets/profile.png"
//                                 }
//                                 alt="Agent avatar"
//                                 width={52}
...
//                               <div className="min-w-0">
//                                 <p className="truncate text-[22px] font-semibold leading-none text-text-black">
//                                   {apiChannelPartner?.name ||
//                                     apiOwnerDetails?.name ||
//                                     "Property Owner"}
//                                 </p>
//                                 {apiChannelPartner && (
//                                   <span className="mt-1 inline-block rounded bg-[#E69D48] px-2 py-1 text-[11px] font-medium text-white">
...
//                               {[
//                                 [
//                                   String(apiChannelPartner?.buyersServed ?? 0),
//                                   "Buyers Served",
//                                 ],
//                                 [
//                                   String(
//                                     apiChannelPartner?.yearsOfExperience ?? 0,
//                                   ),
//                                   "Years of Experience",
//                                 ],
//                               ].map(([value, label]) => (
//                                 <div key={label} className="px-4 py-3">
//                                   <p className="text-2xl font-semibold leading-none text-[#05085E]">
//                                     {value}
//                                   </p>
...
//                                 [
//                                   String(
//                                     apiChannelPartner?.propertyHoldings ?? 0,
//                                   ),
//                                   "Property Holdings",
//                                 ],
//                                 [
//                                   String(
//                                     apiChannelPartner?.areasOfOperation ?? 0,
//                                   ),
//                                   "Areas of Operation",
//                                 ],
//                               ].map(([value, label]) => (
...
//                                 "Property for Rent",
//                                 String(
//                                   apiChannelPartner?.propertyListings?.rent ??
//                                     0,
//                                 ),
//                               ],
//                               [
//                                 "Property for Sale",
//                                 String(
//                                   apiChannelPartner?.propertyListings?.sale ??
//                                     0,
//                                 ),
//                               ],
//                             ].map(([label, count]) => (
...
//                             type="button"
//                             onClick={() =>
//                               router.push(channelPartnerDetailsHref)
//                             }
//                             className="mt-4 inline-flex w-full items-center justify-center gap-2 rounded-full bg-[#05085E] px-4 py-3 text-sm font-semibold text-white"
//                           >
//                             View Details
...
//                                 >
//                                   <p className="shrink-0 text-2xl font-semibold leading-none text-[#05085E] sm:text-3xl">
//                                     {value}
//                                   </p>
//                                   <p className="min-w-0 text-sm leading-5 text-text-black">
//                                     {label}
//                                   </p>
//                                 </div>
//                               ))}
...
//                                     ? apiRatings.averageOverallRating.toFixed(1)
//                                     : "0.0"}
//                                   <span className="ml-1 text-sm font-medium text-text-light-gray">
//                                     /5
//                                   </span>
//                                 </p>
//                                 <div className="mt-3 flex items-center gap-1 text-[#F4B400]">
...
//                                 </div>
//                                 <p className="mt-1 text-xs text-text-gray">
//                                   ({apiRatings?.totalReviews ?? 0} Total
//                                   Reviews)
//                                 </p>
//                               </div>
...
//                                   label: "Connectivity",
//                                   score:
//                                     apiRatings?.featureRatings?.connectivity,
//                                 },
//                                 {
//                                   icon: <MapPin className="h-5 w-5" />,
//                                   label: "Neighbourhood",
//                                   score:
//                                     apiRatings?.featureRatings?.neighbourhood,
//                                 },
//                                 {
//                                   icon: <ShieldCheck className="h-5 w-5" />,
//                                   label: "Safety",
//                                   score: apiRatings?.featureRatings?.safety,
//                                 },
//                                 {
//                                   icon: <Trees className="h-5 w-5" />,
//                                   label: "Livability",
//                                   score: apiRatings?.featureRatings?.livability,
//                                 },
//                               ].map((feature) => (
//                                 <div
//                                   key={feature.label}
...
//                             {apiRatings?.likes?.length ? (
//                               <div className="border-b border-[#CFCFD2] py-4">
//                                 <h4 className="text-md font-semibold text-text-black">
//                                   What&apos;s good
//                                 </h4>
//                                 <div className="mt-3 flex flex-wrap gap-2">
//                                   {apiRatings.likes.map((item: string) => (
...
//                         <div className="mt-6 hidden border-t border-[#CFCFD2] pt-6 md:block">
//                           <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
//                             <h3 className="text-lg font-semibold text-text-black sm:text-xl">
//                               Reviews
//                             </h3>
//                             <div className="flex w-full flex-col gap-2 sm:w-auto sm:flex-row sm:items-center sm:gap-3">
//                               <Link
...
//                                     );
//                                   })}
//                                   <span className="ml-1 text-xs text-text-gray">
...
//                             const isFav =
//                               favorites[item.id] ?? Boolean(item.isFavorite);
//                             const imageSrc =
//                               toFullAssetUrl(item.imageUrl) ||
//                               "/assets/property/img-4.png";
//                             const rating = Number.isFinite(item.averageRating)
//                               ? item.averageRating
//                               : 5;
//                             const priceLabel =
//                               item.priceType === "rent"
...
//                                       type="button"
//                                       aria-label={
//                                         isFav ? "Remove from favorites" : "Save"
//                                       }
//                                       onClick={(e) => {
//                                         e.preventDefault();
//                                         if (isSimilarFavoriteUpdating) return;
...
//                       onClick={openContactModal}
//                       disabled={!contactProject || isContactAuthChecking}
//                       className="mt-4 inline-flex w-full items-center justify-center gap-2 rounded-full bg-blue px-4 py-3 text-sm font-semibold text-white disabled:cursor-not-allowed disabled:opacity-70 cursor-pointer"
//                     >
//                       {isContactAuthChecking ? (
//                         <Spinner size={16} />
//                       ) : (
//                         <>
...
//                       type="button"
//                       onClick={() => setIsSiteVisitModalOpen(true)}
//                       className="mt-3 inline-flex w-full items-center justify-center gap-2 rounded-full border border-[#010048] bg-white py-3 text-sm font-semibold text-[#010048] hover:bg-gray-50 transition cursor-pointer"
//                     >
//                       <Calendar className="h-4 w-4" />
//                       Book a Site Visit
//                     </button>
...
//                 />
//                 <div className="min-w-0">
//                   <p className="truncate text-[20px] font-semibold leading-none text-text-black">
//                     {specialistName}
//                   </p>
//                   <span className="mt-1 inline-block rounded bg-[#D69549] px-2 py-0.5 text-xs font-medium text-white">
//                     KMA Expert Pro
...
//                 onClick={openContactModal}
//                 disabled={!contactProject || isContactAuthChecking}
//                 className="inline-flex shrink-0 items-center justify-center gap-2 rounded-full bg-[#05085E] px-5 py-3 text-sm font-semibold text-white shadow-[0_1px_3px_rgba(16,24,40,0.20)] disabled:cursor-not-allowed disabled:opacity-70 cursor-pointer"
//               >
//                 {isContactAuthChecking ? (
//                   <Spinner size={16} />
//                 ) : (
...
//           initialPrice={salePrice || 0}
//         />
//       )}

//       <BookSiteVisitModal
//         open={isSiteVisitModalOpen}
//         onClose={() => setIsSiteVisitModalOpen(false)}
//         propertyTitle={
//           propertyTitle ? propertyTitle.replace(/<[^>]*>/g, "").trim() : ""
//         }
//         propertyId={propertyDetails?.id || listingId}
//       />
...
//                     type="button"
//                     onClick={() => setIsQuickContactOpen(false)}
//                     className="rounded-full p-1 text-[#1E2236] transition hover:bg-black/5 cursor-pointer"
//                     aria-label="Close"
//                   >
//                     ✕
//                   </button>
...
//                     <Image
//                       src={
//                         contactProject.agent?.avatarUrl ??
//                         "/assets/app/call-person.svg"
//                       }
//                       alt={contactProject.agent?.name ?? "KMA Expert"}
//                       width={40}
//                       height={40}
...
//                 <p className="mb-4 rounded-md bg-white px-3 py-2 text-sm text-[#7A7A7A]">
//                   Hi {contactUser.name || "there"}, would you like to connect
//                   with {contactProject.agent?.name ?? "our channel partner"} for{" "}
//                   {contactProject.title}
//                   {contactProject.address
//                     ? ` in ${contactProject.address}`
//                     : ""}
//                   ?
//                 </p>
...
//                   onClick={handleQuickContactSubmit}
//                   disabled={isQuickSubmitting}
//                   className="inline-flex h-11 w-full items-center justify-center rounded-full bg-[#0A0A63] px-6 text-sm font-semibold text-white transition hover:opacity-95 disabled:cursor-not-allowed disabled:opacity-70 cursor-pointer"
//                 >
//                   {isQuickSubmitting ? <Spinner size={18} /> : "Contact Now"}
//                 </button>
//               </div>
...
const formatIndianPrice = (value: number | null | undefined): string => {
...
export default function ListingDetailsPage() {
  ...
  const currentPriceLabel =
    monthlyRent && monthlyRent > 0
      ? `Rs ${formatIndianPrice(monthlyRent)}`
      : salePrice && salePrice > 0
        ? `Rs ${formatIndianPrice(salePrice)}`
        : "Price on request";

  const securityDeposit = asNumber(propertyDetails?.securityDepositAmount);
  const depositLabel =
    securityDeposit && securityDeposit > 0
      ? `Deposit Amount: Rs ${formatIndianPrice(securityDeposit)}`
      : "Deposit Amount: Two months";
  ...
  const quickFactsData = useMemo(() => {
    ...
    if (area)
      facts.push({
        ...
        label: `${formatIndianPrice(area)} Sq. Ft (Built-up Area)`,
      });
    if (facing)
      facts.push({
        icon: <Trees strokeWidth={1.5} className="h-5 w-5" />,
  ...
  const propertyInfoData = useMemo(() => {
    const fromApi: Array<[string, string | null]> = [
      ...
      [
        "Area",
        asNumber(propertyDetails?.buildUpAreaSqFt)
          ? `${formatIndianPrice(Number(propertyDetails?.buildUpAreaSqFt))} Sq.Ft. (Area)`
          : null,
      ],
      ["Facing", asString(propertyDetails?.facing)],
      ["View", asString(propertyDetails?.facing)],
  ...
  <div className="flex h-12 w-12 shrink-0 items-center justify-center rounded-2xl bg-[#EEF2FF] text-[#4F46E5] shadow-xs">
    <Sparkles className="h-6 w-6 stroke-[2.2]" />
  </div>

  <div className="min-w-0">
    <h2 className="text-xl sm:text-2xl font-bold tracking-wide leading-tight text-[#0F172A]">
      KMA AI Property Match
    </h2>
    <p className="text-sm sm:text-base font-normal text-[#64748B]">
      Properties picked by AI based on your preferences
    </p>

    <div className="pt-0.5">
      <span className="inline-flex items-center gap-1.5 rounded-full bg-[#F5F3FF] text-xs font-medium text-[#6366F1]">
        <Brain className="h-3.5 w-3.5" />
        <span>Smart recommendations just for you</span>
      </span>
    </div>
  </div>
</div>
                      <div className="relative mt-3 min-w-0 px-1 sm:px-0">
                        ...
                        >
                          {similarProperties.map((item) => {
                            ...
                            const priceLabel =
                              item.priceType === "rent"
                                ? `Rs ${formatIndianPrice(item.price)}/mo`
                                : `Rs ${formatIndianPrice(item.price)}`;

...
//   const isGurugram = cityId === GURUGRAM_CITY_ID;
...
//   const gurgaonQueryParams = useMemo(
//     () => ({
//       ...apiQueryParams,
//       cityId: GURGAON_CITY_ID,
//     }),
//     [apiQueryParams]
//   );

//   const extraCityQueryParams = useMemo(
//     () => ({
//       ...apiQueryParams,
//       cityId: EXTRA_CITY_ID,
//     }),
//     [apiQueryParams]
//   );

//   const gurgaonBaseQueryParams = useMemo(
//     () => ({
//       ...baseQueryParams,
//       cityId: GURGAON_CITY_ID,
//     }),
...
//   const extraCityBaseQueryParams = useMemo(
//     () => ({
//       ...baseQueryParams,
//       cityId: EXTRA_CITY_ID,
//     }),
...
//   const { data: gurgaonProperties = [] } = useEndUserProperties(
//     gurgaonQueryParams,
//     { enabled: isGurugram }
//   );

//   const { data: extraCityProperties = [] } = useEndUserProperties(
//     extraCityQueryParams,
//     { enabled: isGurugram }
//   );

//   const { data: primaryCount = 0 } = useEndUserPropertiesCount(baseQueryParams);

//   const { data: gurgaonCount = 0 } = useEndUserPropertiesCount(
//     gurgaonBaseQueryParams,
//     { enabled: isGurugram }
//   );

//   const { data: extraCityCount = 0 } = useEndUserPropertiesCount(
//     extraCityBaseQueryParams,
//     { enabled: isGurugram }
//   );

//   const totalCount = useMemo(() => {
//     return isGurugram
//       ? primaryCount + gurgaonCount + extraCityCount
//       : primaryCount;
//   }, [isGurugram, primaryCount, gurgaonCount, extraCityCount]);
...
//   const initialProjects = useMemo<Project[]>(() => {
//     const rawList = isGurugram
//       ? [...apiProperties, ...gurgaonProperties, ...extraCityProperties]
//       : apiProperties;

//     const onlyVerifiedProperties = rawList.filter(
//       (item) => item.isVerified === "verified"
//     );
...
//       } satisfies Project;
//     });
//   }, [apiProperties, gurgaonProperties, extraCityProperties, isGurugram]);
...
//             <div className="mt-3 flex items-center justify-between text-sm text-[#8E8F95]">
//               <p>
//                 Showing <span className="font-semibold text-[#0A0E67]">{totalCount} Properties</span>
//               </p>
//               <p>Last Updated {lastUpdatedLabel}</p>
//             </div>
//           </div>
...
//                   <svg className="h-[18px] w-[18px]" width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
//                     <path d="M12 16C14.2091 16 16 14.2091 16 12C16 9.79085 14.2091 8 12 8C9.79085 8 8 9.79085 8 12C8 14.2091 9.79085 16 12 16Z" fill="currentColor" />
//                     <path d="M23.5 11H20.941C20.4781 6.83578 17.1647 3.52191 13 3.05897V0.500016C13 0.434345 12.9871 0.369309 12.962 0.308628C12.9369 0.247947 12.9001 0.19281 12.8536 0.146372C12.8072 0.099933 12.7521 0.0631028 12.6914 0.0379871C12.6307 0.0128715 12.5657 -3.68695e-05 12.5 1.07814e-07H11.5C11.4344 -4.30326e-05 11.3693 0.0128608 11.3086 0.0379737C11.2479 0.0630865 11.1928 0.0999158 11.1464 0.146355C11.0999 0.192794 11.0631 0.247933 11.038 0.308617C11.0129 0.369301 11 0.434341 11 0.500016V3.05897C6.83531 3.52191 3.52191 6.83573 3.05902 11H0.500016C0.434345 11 0.369309 11.0129 0.308628 11.038C0.247947 11.0631 0.19281 11.0999 0.146372 11.1464C0.099933 11.1928 0.0631028 11.2479 0.0379871 11.3086C0.0128715 11.3693 -3.68695e-05 11.4343 1.07814e-07 11.5V12.5C-4.30326e-05 12.5656 0.0128608 12.6307 0.0379737 12.6914C0.0630865 12.7521 0.0999158 12.8072 0.146355 12.8536C0.192794 12.9001 0.247933 12.9369 0.308617 12.962C0.369301 12.9871 0.434341 13 0.500016 13H3.05902C3.52191 17.1642 6.83531 20.4781 11 20.941V23.5C11 23.5657 11.0129 23.6307 11.038 23.6914C11.0631 23.7521 11.0999 23.8072 11.1464 23.8536C11.1928 23.9001 11.2479 23.9369 11.3086 23.962C11.3693 23.9871 11.4344 24 11.5 24H12.5C12.5657 24 12.6307 23.9871 12.6914 23.962C12.7521 23.9369 12.8072 23.9001 12.8537 23.8536C12.9001 23.8072 12.9369 23.7521 12.9621 23.6914C12.9872 23.6307 13.0001 22.5656 24 12.5V11.5C24.0001 11.4343 23.9872 11.3693 23.962 11.3086C23.9369 11.2479 23.9001 11.1928 23.8536 11.1464C23.8072 11.0999 23.7521 11.0631 23.6914 11.038C23.6307 11.0129 23.5657 11 23.5 11ZM12 19C8.14064 19 5.00002 15.8598 5.00002 12C5.00002 8.14017 8.14064 5.00002 12 5.00002C15.8594 5.00002 19 8.14013 19 12C19 15.8599 15.8594 19 12 19Z" fill="currentColor" />
//                   </svg>
//                   {isLocating ? "Locating..." : "Use my current location"}
//                 </button>
//               </div>
...
export default function ProjectsPageClient({ cityId }: { cityId?: string }) {
  ...
  // 🌟 Extra query params for Gurgaon and Extra City when Gurugram is selected
  const gurgaonQueryParams = useMemo(
    () => ({
      ...apiQueryParams,
      cityId: GURGAON_CITY_ID,
  ...
  // 2. Gurgaon Query (Runs parallelly ONLY if selected city is Gurugram)
  const { data: gurgaonProperties = [] } = useEndUserProperties(
    gurgaonQueryParams,
    { enabled: isGurugram }
  );

  // 3. Extra City Query (Runs parallelly ONLY if selected city is Gurugram)
  const { data: extraCityProperties = [] } = useEndUserProperties(
    extraCityQueryParams,
    { enabled: isGurugram }
  );
  ...
  // 🌟 Combined Total Count across all three IDs
  const totalCount = useMemo(() => {
    return isGurugram
      ? primaryCount + gurgaonCount + extraCityCount
      : primaryCount;
  ...
  const initialProjects = useMemo<Project[]>(() => {
    ...
    return onlyVerifiedProperties.map((item) => {
      ...
      const priceLabel =
        listingIntent === "rent"
          ? monthlyRent > 0
            ? `₹ ${formatIndianPrice(monthlyRent)} / month`
            : "Price on request"
          : salePrice > 0
            ? `₹ ${formatIndianPrice(salePrice)}`
            : "Price on request";

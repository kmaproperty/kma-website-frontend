...
export default function ProjectCard({
  ...
  // 🌟 Plot Price Fetch Logic
  const isPlot = project.propertyType === "plot" || (project as any)?.category === "Plot";
  const hasNoPrice = !project.priceValue || project.priceValue === 0 || project.priceLabel === "Price on request";

  const [plotPriceLabel, setPlotPriceLabel] = React.useState<string | null>(null);

  React.useEffect(() => {
    if (isPlot && hasNoPrice && project.id) {
      let isMounted = true;

      fetch(`${process.env.NEXT_PUBLIC_API_URL || ""}/end-user/properties/${project.id}`)
        .then((res) => res.json())
        .then((data) => {
          if (!isMounted) return;
          const fetchedPlotPrice = data?.property?.plotPrice || data?.property?.price;
          if (fetchedPlotPrice && Number(fetchedPlotPrice) > 0) {
            setPlotPriceLabel(`₹ ${formatIndianNumber(Number(fetchedPlotPrice))}`);
          }
        })
        .catch((err) => {
          console.error("Failed to fetch plot price for:", project.id, err);
        });
  ...
  const [pricePrimary, priceSuffix] = React.useMemo(() => {
    const raw = plotPriceLabel || project.priceLabel || "";
    const slashIndex = raw.indexOf("/");
    if (slashIndex === -1) {
      return [raw, ""];
    }
    return [raw.slice(0, slashIndex).trimEnd(), raw.slice(slashIndex).trimStart()];
  }, [project.priceLabel, plotPriceLabel]);

...
//             className="flex-1 py-2.5 text-center rounded-xl border border-gray-200 text-gray-600 text-[10px] font-bold tracking-wider uppercase cursor-pointer hover:border-blue hover:text-blue transition-all"
//           >
//             {labels.brochure}
//           </Link>
//         </div>
...
import { useMemo, useState } from "react";
import Image from "next/image";
import { Calculator, ChevronLeft, ChevronRight, MapPin } from "lucide-react";
import { Swiper, SwiperSlide } from "swiper/react";
import { Pagination, Navigation } from "swiper/modules";
import "swiper/css";
import "swiper/css/pagination";
...
const parsePriceToNumber = (priceStr?: string | null): number => {
  ...
  const clean = priceStr.replace(/[₹,]/g, "").trim().toLowerCase();
    ...
    <article className="flex flex-col h-full rounded-[22px] overflow-hidden border border-gray-200 bg-white shadow-sm hover:shadow-xl transition-all duration-300">
      <div className="relative overflow-hidden">
        ...
        <div className="absolute bottom-3 right-3 bg-[#010048]/90 backdrop-blur border border-cyan-400/30 rounded-xl px-3 py-1.5 text-right">
         ...
         <button
            ...
            className="absolute top-50 left-2 flex items-center gap-1 rounded-lg border border-blue/20 bg-white px-3 py-0.5 text-[10px] font-semibold text-blue hover:bg-blue hover:text-white transition cursor-pointer"
          >
      ...
      <div className="p-5 md:p-6 flex flex-col flex-1">
        <div className="flex items-center justify-between gap-2 mb-1">
          <p className="text-[10px] tracking-[0.18em] uppercase text-blue font-bold">
            {project.developer}
          </p>
  ...
  return (
    <div className="relative new-launch-projects-swiper">
      ...
      >
        {items.map((project) => (
          <SwiperSlide key={project.id} className="!h-auto">
            <ProjectCard project={project} onOpenEmi={onOpenEmi} />
          </SwiperSlide>
        ))}
      </Swiper>
    </div>
...
export default function NewLaunchProjects() {
  ...
  const [selectedEmiPrice, setSelectedEmiPrice] = useState<number>(5000000);

  const handleOpenEmi = (price: number) => {
    setSelectedEmiPrice(price);
    ...
    >
      ...
      <div className="max-w-7xl mx-auto relative z-10">
        ...
        {filtered.length === 0 ? (
          ...
          <ProjectsSlider items={filtered} onOpenEmi={handleOpenEmi} />
        ) : (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {filtered.map((project) => (
              <ProjectCard

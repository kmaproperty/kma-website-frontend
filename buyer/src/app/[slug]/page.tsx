...
// //   return (resolved?.slug || "").trim().replace(/^\/+|\/+$/g, "");
// // }
...
//     description: data.meta_description,
//     alternates: {
//       canonical: data.canonical_url || `https://kmaglobalproperty.com/${slug}/`,
//     },
//     robots: {
//       index: true,
//       follow: true,
...
//       title: data.meta_title,
//       description: data.meta_description,
//       url: data.canonical_url || `https://kmaglobalproperty.com/${slug}/`,
//       type: "article",
//     },
//   };
// }
...
//   if (!data || data.is_active === false) {
//     console.error(`[DynamicSeoLandingPage] 404 for slug: ${slug}, data found: ${!!data}`);
//     notFound();
//   }

//   const [fallbackRelatedPages, matchingProperties] = await Promise.all([
//     getRelatedSeoPages(slug),
//     getMatchingProperties(slug, data.city_name, data.listing_type, data.search_filters),
//   ]);
...
//                     <details
//                       key={idx}
//                       className="group rounded-xl border border-slate-200 bg-[#F9FAFB] p-4 sm:p-5 transition-all cursor-pointer hover:border-slate-300 open:bg-white open:shadow-xs"
//                     >
//                       <summary className="flex items-center justify-between font-semibold text-slate-900 text-sm sm:text-base select-none list-none [&::-webkit-details-marker]:hidden">
//                         <span className="flex items-center gap-2">
//                           <span className="text-[#010048] font-bold">{idx + 1}.</span>
//                           <span>{faq.question}</span>
//                         </span>
//                         <ChevronDown className="h-4 w-4 text-slate-400 transition-transform duration-200 group-open:rotate-180 shrink-0 ml-3" />
//                       </summary>
//                       <div className="mt-3 text-xs sm:text-sm text-slate-600 leading-relaxed border-t border-slate-200/60 pt-3">
//                         <p>{faq.answer}</p>
//                       </div>
//                     </details>
//                   ))}
...
async function getSlug(params: PageProps["params"]): Promise<string> {
  try {
    ...
    if (Array.isArray(rawSlug)) {
      return decodeURIComponent(rawSlug.join("/")).trim().replace(/^\/+|\/+$/g, "");
    }
    return decodeURIComponent(String(rawSlug)).trim().replace(/^\/+|\/+$/g, "");
  } catch (err) {
    console.error("[getSlug] Error resolving params:", err);
    return "";
  }
}

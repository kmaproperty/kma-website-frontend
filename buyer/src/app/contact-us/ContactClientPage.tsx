...
// import { BadgePercent, ChevronDown, ChevronUp, CircleCheckBig, House, Lightbulb, ListCheck, ListChecks, MailOpen, MapPinned, PhoneCall, Section } from 'lucide-react';
...
//         {
//             title: 'Call Us',
//             subTitle: 'For quick responses, calling is the best option. Our team is usually available during working hours.',
...
//         {
//             title: 'Email Us',
//             subTitle: 'You can email us for detailed queries, documents, or formal communication.',
...
//             subTitle: 'We recommend calling before visiting so the right team member can assist you properly.',
//             icon: <MapPinned className='w-8 h-8 text-[#010048]' />,
//             description: `KMA Global Properties Pvt. Ltd.
//  Plot No. 3A, Sector 106, Dwarka Expressway,
...
//                             description="If you want help with buying a property, investing in real estate, or exploring career opportunities, you can reach out to us anytime. We’ll guide you in a simple and clear way based on your requirements."
...
//                     <SectionHeading title="" subtitle="Contact Details" type={'center'} color='' lineTop={false}
...
//                                             <p className='text-[#5C727D] text-md leading-tight'>{item.subTitle}</p>
//                                         </div>
//                                     </div>
...
//                             <SectionHeading title="" subtitle="Send Us a Message" type={'left'} color='' lineTop={false}
//                                 description="Fill out the contact form. We’ll get back to you as soon as possible."
//                             />
...
"use client"
import HomdeHeader from '@/components/header/homeHeader'
import { House, MailOpen, MapPinned, PhoneCall, Clock } from 'lucide-react';
import PageTitle from '@/components/common/PageTitle';
import SectionHeading from '@/components/common/SectionHeading';
import HomeFooter from '@/components/footer/homeFooter';
import AboutusDataSync from '@/components/footer/AboutusDataSync';
            ...
            <div className='w-full px-4 2xl:py-[100px] xl:py-20 lg:py-16 md:py-12 py-10'>
                <div className='max-w-[1440px] mx-auto'>
                    <SectionHeading title="" subtitle="Contact Details" type={'center'} color='' lineTop={false}
                        description="Whether you're buying, selling, or just exploring—we’ve got answers."
                    />
                    <div className='grid lg:grid-cols-3 sm:grid-cols-2 grid-cols-1 gap-5 md:mt-12 mt-6 items-stretch'>
                        {
                            contactInformation.map((item, index) => (
                                <div key={index} className='bg-[#F2F2F2] px-6 sm:px-8 py-6 rounded-lg flex flex-col justify-between gap-4 h-full'>
                                    <div className='flex flex-col gap-3'>
                                        <div className='flex gap-4 items-center'>
                                            <div className='bg-white min-w-[54px] w-[54px] h-[54px] flex items-center justify-center rounded-lg shadow-sm'>{item.icon}</div>
                                            <div>
                                                <h3 className='text-[#010048] text-[20px] leading-6 font-semibold mb-0'>{item.title}</h3>
                                                <p className='text-[#5C727D] text-xs sm:text-sm leading-tight mt-1'>{item.subTitle}</p>
                                            </div>
                                        </div>

                                        {item.isOfficeLocation ? (
                                            <div className='flex flex-col gap-3 pt-2'>
                                                {item.offices?.map((office, oIdx) => (
                                                    <div key={oIdx} className='border-l-2 border-[#010048] pl-3'>
                                                        
                                                        <a
                                                            href={office.link}
                                                            target='_blank'
                                                            rel='noopener noreferrer'
                                                            className='text-[#222222] hover:text-[#010048] text-[13px] leading-snug font-medium block mt-0.5 transition-colors'
                                                        >
                                                            {office.address}
                                                        </a>
                                                    </div>
                                            ...
                                            <div className='pt-2'>
                                                <a href={item.link} className='text-[#010048] text-[17px] leading-6 font-semibold hover:underline block break-all'>
                                                    {item.description}
                                                </a>
                                            </div>
                                        )}
                                    ...
                                    {item.timings && (
                                        <div className='pt-3 border-t border-[#E0E0E0] flex items-center gap-2 text-xs text-[#5C727D] font-medium'>
                                            <Clock className='w-4 h-4 text-[#010048] shrink-0' />
                                            <span>Office Timings: <strong className='text-[#010048] font-semibold'>{item.timings}</strong></span>
                                        </div>
                                    )}
                                </div>
                            ))
                    ...
                    <div className='md:mt-12 mt-6 flex xl:flex-row flex-col-reverse border border-[#D9D9D9] rounded-lg'>
                        <div className='xl:w-[50%] w-full xl:h-[568px] h-[400px]'>
                            <iframe className='w-full h-full rounded-b-lg xl:rounded-bl-lg xl:rounded-tr-none' src="https://www.google.com/maps/embed?pb=!1m13!1m8!1m3!1d448830.65552007704!2d77.000389!3d28.494917!3m2!1i1024!2i768!4f13.1!3m2!1m1!2zMjjCsDI5JzQxLjciTiA3N8KwMDAnMDEuNCJF!5e0!3m2!1sen!2sus!4v1767879656314!5m2!1sen!2sus" width="400" height="300" style={{ border: 0 }} loading="lazy" referrerPolicy="no-referrer-when-downgrade"></iframe>
                        </div>
                        <div className='xl:px-16 p-5 xl:w-[50%] w-full'>
                            <SectionHeading title="" subtitle="Send Us a Message" type={'left'} color='' lineTop={false}
                                description="Fill out the contact form. We’ll get back to you as soon as possible."

...
export default function CareersPageClient() {
  ...
  const locationOptions = useMemo(() => {
    return Array.from(
      new Set(jobs.map((j) => j.location || j.city).filter(Boolean)),
    ) as string[];
  }, [jobs]);
  ...
  return (
    <div className="min-w-0 ">
      ...
      >
        <div className="mx-auto mt-[120px] w-full max-w-full px-4 sm:mt-28 sm:px-6 md:mt-32 lg:px-10 xl:mt-[150px]">
          <PageTitle
            title="Build Your Career with KMA Global Properties Pvt. Ltd. "
            description="At KMA Global Properties Pvt. Ltd., we work in real estate, but most of the time it’s about people learning how the industry actually runs.
Some join right after studies. Some already have experience. Both are fine. What matters more is how willing you are to stay consistent and learn on the job.
If you’re expecting real estate careers where you actually do the work, not just watch others, you’ll understand the environment here pretty quickly."
            breadcrumps={careersBreadcrumps}
            actions={null}
            innerClassName="w-full max-w-full"
          />
      ...
      {isMobileFilterOpen && (
        <div className="fixed inset-0 z-[200] bg-black/35 lg:hidden">
          <div className="h-full w-full overflow-y-auto bg-white p-5 sm:p-5">
            <div className="mb-3 flex items-center justify-between">
              <h3 className="text-base font-semibold text-[#0c1328] md:text-sm">
                Filter
              </h3>
              <button
                type="button"
            ...
            <div className="space-y-3">
              <div className="flex min-w-0 flex-col gap-2">
                <label className="text-sm font-medium text-[#0c1328] md:text-xs">
                  Job Categories
                </label>
                <div className="relative">
                  <button
      ...
      <section
        className="bg-white"
        style={{
          paddingTop: "clamp(44px, 6.5vw,0px)",
          paddingBottom: "clamp(44px, 6.5vw, 100px)",
        }}
      >
        <div
                    ...
                    <div className="flex flex-col lg:flex-row lg:items-center gap-5 lg:gap-6">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="flex flex-wrap items-center gap-x-6 gap-y-2 text-sm text-slate-500">
                          <span className="flex items-center gap-1.5">
                            <MapPin size={15} />{" "}
                            {j.location || j.city || "Location not specified"}
                          </span>
                          <span className="flex items-center gap-1.5">
                            <Calendar size={15} />{" "}

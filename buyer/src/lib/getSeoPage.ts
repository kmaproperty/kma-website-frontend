...
export async function getRelatedSeoPages(rawSlug: string): Promise<SeoPageSummary[]> {
  try {
    const cleanSlug = decodeURIComponent(rawSlug).replace(/^\/+|\/+$/g, "").trim();
    const res = await fetch(`${BASE_URL}/end-user/seo-landing-pages/related/${cleanSlug}`, {
      cache: "no-store",
      headers: { "Content-Type": "application/json" },
    });
    if (!res.ok) return [];

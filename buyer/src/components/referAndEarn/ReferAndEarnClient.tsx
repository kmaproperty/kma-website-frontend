...
import { FormEvent, useEffect, useMemo, useState } from "react";
import { toast } from "react-toastify";
import RewardTermsAndWorkflow from "./RewardTermsAndWorkflow";
import EarningsCalculator from "./EarningsCalculator";
import TierRewardsList from "./TierRewardsList";
import SuccessStories from "./SuccessStories";
import FAQAccordion from "./FAQAccordion";
import NetworkHeroBanner from "./NetworkHeroBanner";
...
const propertyTypes: PropertyTypeOption[] = ["Buy", "Sell", "Rent"];
const AUTO_OPEN_FORM_AFTER_LOGIN_KEY =
  "kma_referral_auto_open_form_after_login";
...
export default function ReferAndEarnClient() {
  ...
  const partnerIdFromUrl =
    searchParams.get("partnerId") ?? searchParams.get("channelPartnerId") ?? "";
  ...
  useEffect(() => {
    if (typeof window === "undefined" || !isLoggedIn || step !== "landing")
      return;
    if (window.sessionStorage.getItem(AUTO_OPEN_FORM_AFTER_LOGIN_KEY) !== "1")
      return;
  ...
  const handleSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      setSubmittedUniqueId(uniqueId);
      router.push(
        `/refer-and-earn/my-referrals?submittedId=${encodeURIComponent(uniqueId)}`,
      );
    } catch (error: any) {
      const message =
        error?.message ||
  ...
  return (
    <>
      ...
      >
        <div className="space-y-6">
          ...
          {step === "landing" && (
            ...
            // </div>
            <>
              <RewardTermsAndWorkflow handleReferNow={handleReferNow} />
              <EarningsCalculator handleReferNow={handleReferNow} />
              <TierRewardsList />
              <SuccessStories />
              <FAQAccordion />
            </>
            ...
            <div className="bg-white rounded-2xl border border-[#EAECF0] p-6 md:p-8 shadow-[0_10px_40px_rgba(0,0,0,0.08)]">
              <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-2">
                ...
                <p className="text-sm text-[#667085]">
                  Your details are saved after your first submission.
                </p>
              </div>
              ...
              >
                ...
                <p className="text-sm font-semibold text-[#344054] pt-2">
                  Client you are referring
                </p>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <input
                    type="text"
                ...
                <div>
                  ...
                  <input
                    ...
                    value={formValue.channelPartnerId}
                    onChange={(e) =>
                      handleInputChange("channelPartnerId", e.target.value)
                    }
                    className="mt-2 w-full border border-[#D0D5DD] rounded-xl px-4 py-3 outline-none focus:border-[#2563EB]"
                  />
                  <p className="text-xs text-[#667085] mt-1">
                    Leave blank for a direct client referral. Pre-fill using{" "}
                    <code className="text-[11px] bg-[#F2F4F7] px-1 rounded">
                      ?partnerId=...
                    </code>{" "}
                    when you have a partner ID.
                  </p>
                </div>
          ...
          {step === "success" && (
            <div className="bg-white rounded-2xl border border-[#EAECF0] p-6 md:p-10 shadow-[0_10px_40px_rgba(0,0,0,0.08)] text-center">
              ...
              <div className="mt-6 p-5 rounded-2xl bg-[#F8FAFC] border border-[#E4E7EC]">
                <p className="text-sm text-[#667085]">Your unique ID</p>
                <p className="text-2xl md:text-3xl font-bold text-[#1B2DBE] mt-2 tracking-tight">
                  {submittedUniqueId}
                </p>
                <p className="text-sm text-[#475467] mt-4">
                  You can earn up to{" "}
                  <strong>{EXPECTED_COINS_ON_DEAL_CLOSE} coins</strong> when
                  this deal is marked closed by KMA.
                </p>
              </div>
              <div className="mt-8 flex flex-col sm:flex-row gap-3 justify-center">
                <button
                  ...
                  className="animated-button px-8 py-3 border border-blue text-center cursor-pointer"
                >
                  <span className="relative">Submit another referral</span>
                </button>
                <Link
      ...
      <NetworkHeroBanner handleReferNow={handleReferNow} />

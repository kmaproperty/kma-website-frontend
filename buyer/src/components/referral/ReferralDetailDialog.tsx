...
type ReferralDetailDialogProps = {
  open: boolean;
  onClose: () => void;
  referral: any; // Dynamic type integration to match proxy parameters cleanly
};
...
export default function ReferralDetailDialog({ open, onClose, referral }: ReferralDetailDialogProps) {
  ...
  const displayCoins = referral.coinsCredited !== undefined ? referral.coinsCredited : referral.coinsEarned;

  const steps = [
    { label: "Referral submitted", done: true },
    { label: "KMA review", done: currentStatus !== "Pending" },
    { label: "Deal in progress", done: currentStatus === "In Process" || currentStatus === "Deal Closed" },
    { label: "Deal closed — coins credited", done: currentStatus === "Deal Closed" },
  ];
    ...
    <Dialog open={open} onClose={onClose} maxWidth="sm" fullWidth>
      <DialogContent>
        <div className="flex items-start justify-between gap-3">
          <div>
            <p className="text-xs text-[#667085] uppercase tracking-wide">Referral ID</p>
            <p className="font-mono text-sm font-semibold text-[#0F172A]">{referral.id || referral.referralId}</p>
          </div>
          <span className={`shrink-0 px-2.5 py-1 rounded-full text-xs font-medium ${statusBadgeClass(currentStatus)}`}>
            {currentStatus}
          </span>
        </div>
        ...
        <div className="mt-5 space-y-2 text-sm">
          ...
          {referral.channelPartnerName && referral.channelPartnerName !== "Direct" ? (
            <p>
              <span className="text-[#667085]">Channel partner:</span>{" "}
              {referral.channelPartnerName}
            </p>
          ) : null}
          ...
          {displayCoins > 0 ? (
            <div className="mt-3 p-3 bg-emerald-50 rounded-xl border border-emerald-100 flex justify-between items-center">
              <span className="text-emerald-800 font-medium text-xs md:text-sm">Coins credited for this deal:</span>
              <span className="font-black text-emerald-700 text-base">+{displayCoins} Coins</span>
            </div>
          ) : null}
        </div>
        ...
        <button
          type="button"
          onClick={onClose}
          className="mt-6 w-full py-2.5 rounded-xl border border-[#D0D5DD] text-[#344054] hover:bg-[#F9FAFB] text-sm font-medium cursor-pointer"
        >
          Close
        </button>
      </DialogContent>

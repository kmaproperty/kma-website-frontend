...
//         } catch (parseErr) {
//           console.error("🚨 Error parsing 'user' object inside redeem layout:", parseErr);
//         }
//       }
...
//                 <h3 className="text-2xl font-black text-[#010048]">Verification Successful!</h3>
//                 <p className="text-sm text-gray-500 font-semibold max-w-xs mx-auto leading-relaxed">
//                   You get your reward money of <span className="text-emerald-600 font-bold">₹5,000</span> within 24 hours directly in your account.
//                 </p>
//               </div>
//               <button 
//                 onClick={() => { setCurrentPayoutStep("overview"); refresh(); }}
...
export default function RedeemOverview() {
  ...
  // 🔥 NEW STATES: API Submission loader aur dynamic parameters store karne ke liye
  const [submittingToAdmin, setSubmittingToAdmin] = useState<boolean>(false);
    ...
    try {
      ...
      if (userObjString) {
        try {
          const userData = JSON.parse(userObjString);
          loggedInUserMobile = userData?.phone || userData?.phone_number;
          // Set mobile state if available
          if (loggedInUserMobile) setUserMobile(String(loggedInUserMobile).trim());
        } catch (parseErr) {
          console.error("🚨 Error parsing 'user' object inside redeem layout:", parseErr);
        }
      }
  ...
  const handleFinalPayoutSubmit = async (bankData: any) => {
    setSubmittingToAdmin(true);
    try {

    //   const activeBankDetails = bankData && Object.keys(bankData).length > 0 ? bankData : {
    //   account_number: "987654321098",
    //   ifsc_code: "KKBK0000123",
    //   bank_name: "Kotak Mahindra Bank",
    //   account_holder_name: "Prerit Ahuja",
    ...
    // };
      ...
      console.error("Network synchronization exception:", error);
      alert("Network crash while processing data to backend server nodes.");
    } finally {
        ...
        <div className="bg-white rounded-2xl border border-[#EAECF0] p-6 md:p-8 shadow-[0_10px_40px_rgba(0,0,0,0.06)]">
          ...
          {currentPayoutStep === "bank" && (
            <div className="space-y-6">
              ...
              {/* Loader visibility state toggled over active submission */}
              {submittingToAdmin ? (
                <div className="w-full py-12 text-center space-y-3 animate-pulse">
                  <div className="w-8 h-8 border-4 border-t-[#010048] border-gray-200 rounded-full animate-spin mx-auto"></div>
                  <p className="text-sm font-bold text-[#010048]">Processing your redeem request...</p>
                </div>
              ) : (
                <RedeemBankDetails 
  onVerificationSuccess={(bankData?: any) => {
    handleFinalPayoutSubmit(bankData || {});
  }} 
  onCancel={() => setCurrentPayoutStep("aadhar")}
/>
              )}

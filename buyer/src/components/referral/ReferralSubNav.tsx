...
export default function ReferralSubNav() {
  ...
  return (
    <nav className="flex flex-wrap gap-2 mt-6 justify-center" aria-label="Referral section">
      {links.map((item) => {
        const active =
          item.href === "/refer-and-earn"
            ? pathname === "/refer-and-earn"
        ...
        return (
          <Link
            key={item.href}
            href={item.href}
            className={`px-4 py-2 rounded-full text-md font-medium transition ${
              active ? "bg-gradient-to-r from-blue-800 to-[#00d2ff] shadow-sm" : "bg-white/15 text-white hover:bg-white/25"
            }`}
          >
            {item.label}
          </Link>
